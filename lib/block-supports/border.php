...
function gutenberg_has_border_feature_support( $block_type, $feature, $default = false ) {
	...
	if (
		...
		&& (
			( isset( $block_type->supports['__experimentalBorder'] ) && true === $block_type->supports['__experimentalBorder'] )
			|| ( ! isset( $block_type->supports['__experimentalBorder'] ) && true === $default )
		)
	) {
		return true;
	}
